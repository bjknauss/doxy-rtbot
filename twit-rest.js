...
var defaultIntervalPeriod = 1000 * 60 * 5;

var TwitRest = function(twit, intervalPeriod){
    this.twit = twit;
    this.retweetQueue = [];
    this.retweetScheduler = null;
    this.favoriteQueue = [];
    this.favoriteScheduler = null;
    this.intervalPeriod = intervalPeriod || defaultIntervalPeriod;
}

TwitRest.prototype.start = function(){
    var self = this;
    debug("starting retweet and favorite schedulers.");
    self.retweetScheduler = setInterval(self.intervalRetweet.bind(self), this.intervalPeriod);
    self.favoriteScheduler = setInterval(self.intervalFavorite.bind(this), this.intervalPeriod);
}
TwitRest.prototype.stop = function(){
    ...
    if(!_.isNil(this.favoriteScheduler)){
        debug("stopping favorite scheduler.");
        clearInterval(this.favoriteScheduler);
    }
}
...
TwitRest.prototype.intervalFavorite = function(){
    if(this.favoriteQueue.length > 0){
        var tweet = this.favoriteQueue.shift();
        var promise = this.favorite(tweet);
        promise.then((data) => {
            debug("favorited: %s", tweet);
        })
        promise.catch((error) => {
            console.log(error);
        });
...
TwitRest.prototype.favorite = function(tweetId){
    var params = {
        id: tweetId,
        'include_entities': false
    };
    return new Promise(
        (resolve, reject) => {
            this.twit.post('favorites/create', params, (err, data, response)=> {
                if(err){
                    console.log(err);
                    reject(err);
                }else{
...
TwitRest.prototype.queueFavorite = function(tweetId){
    if(!_.includes(this.favoriteQueue, tweetId)){
        debug("favoriteQueued: %s", tweetId);
        this.favoriteQueue.push(tweetId);
    }
}
